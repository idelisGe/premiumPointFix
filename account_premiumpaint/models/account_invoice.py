...
class AccountInvoice(models.Model):
    ...
    def _compute_related_sale_order(self):
        for invoice in self:
            ...
            if not payment_type:
                ...
                invoice.payment_type = 'Contado' if invoice_pt_id in (
                    immediate_pt_id, False) else 'Credito'
    ...
    warehouse_id = fields.Many2one('stock.warehouse', string='Warehouse',
                                   compute='_compute_related_sale_order',
                                   store=True)
    amount_calculate_cost = fields.Monetary(string='Costo',
                                            compute='_compute_related_sale_order',
                                            store=True)
    fiscal_printer_status = fields.Selection([('unsent','Sin enviar'),
                                              ('sent','Enviada'),
                                              ('printed','Impresa')],
                                             'Impresora Fiscal', readonly=True,
                                             default=lambda *a: 'unsent')
    ...
    def send_invoice_proxy(self):
        ...
        for line in self.invoice_line_ids:
            if line.invoice_line_tax_ids:
                tmp = '\n!{:011.2f}{:09.3f}{}'.format(line.price_unit,
                                                      line.quantity,
                                                      line.name.replace('\n','')[:117])
                cmd += tmp.replace('.','')
            else:
